...
`json_array_get_member()` returns a pointer to the member at `index` of an Array or `NULL` should `index` be out-of-bounds.
...
`json_object_search()` searches an Object for a member with a name matching `name`.  If a match is found a pointer to the corresponding member is returned, otherwise, `NULL` is returned.
