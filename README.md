...
1. name (key) associated with the Node.  Could be `NULL`.
2. the current Node.
3. the user-defined `cb_data` parameter.

`exit_cb()` is a user-defined callback function that is called just prior to leaving the current Node.  The parameters are as follows:
